...
class izapQueue extends IzapSqlite {
  ...
  */
  public function move_to_trash($guid) {
    ...
    $this->execute("INSERT INTO video_trash (guid, main_file, title, url, access_id, owner_id, timestamp)
...
//    $queue_db = $this->db_connection;
    ...
    $this->execute("INSERT INTO video_queue (guid, main_file, title, url, access_id, owner_id, timestamp)
  ...
  */
  public function fetch_videos($guid = false) {
    $select = $this->execute("SELECT * FROM video_queue WHERE conversion = ".PENDING." ORDER BY timestamp");
    if(count($select)) {
